...
function createdirectory(foldername) {
    let folderpath = path.join(__dirname, "ipl_2020");
...
function extractMatchResult(html) {
    ...
    for (let i = 0; i < teamstitleArray.length; i++) {
        ...
        for (let j = 3; j <= 4; j += 2) {
            ...
            let playername = seltool(playersArray[j]).find(".batsman-cell.text-truncate.out a").text();
            ...
            // console.log(arrforcurrentplayer);
        }
    }
    console.log(teams);
